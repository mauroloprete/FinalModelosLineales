...
ModeloCompleto %>% 
  ...
  row_spec(c(1,2,4,14,15), bold = T, color = "white", background = "#33D4FF") %>% 
...
car::vif(ModeloCompleto) %>% 
  ...
  row_spec(c(4,5,12), bold = T, color = "black", background = "#CEFF33") %>% 
  kable_material(c("striped", "hover")) %>% 
  kable_styling(
    font_size = 12
  )
...
.panelset[
.panel[.panel-name[Definiciones]

]
.panel[.panel-name[Observaciones influyentes]
.center[
```{r}
ModeloRed %>% 
  cooks.distance() %>% 
...
.panel[.panel-name[Observaciones atípicas]
.center[
