...
Para detectar observaciones atípicas, se construye un vector con los residuos studentisados del modelo y se compara con el umbral definido por
una distribución t y en el caso que la superen, se consideran atípicos.

Se definen los residudos studentisados como :
...
Una vez recorrido todo el procedimiento anteriormente mencionado, llegamos a lo que conocemos como \textbf{Modelo Final}.

El mismo inlcuye a el \textit{Gasto per cápita en policía en 1960} (\textbf{Po1}), la \textit{Desigualdad en el ingreso} (\textbf{Ineq}) , 
un \textit{indice que refleja la escolaridad del estado} (\textbf{Ed}), la \textit{cantidad de hombres entre 14 y 20 años de edad cada 1.000 habitantes} 
(\textbf{M}), \textit{Tasa de desempleo urbana de hombres de 35 a 39 años por 1000 habitantes} (\textbf{U2}) y por último \textit{Probabilidad de encarcelamiento*100} (\textbf{Prob}).   
...
De esta manera, podemos ver que ante una variación en un peso en el gasto de policia en el 1960 la \textit{Tasa de Criminalidad} se esperaría que hubiera aumentado en 11.50, dejando
las demás variables constantes. También podemos concluir que en estados donde existe una mayor desigualdad en el ingreso, se espera que la tasa de criminalidad sea mayor, lo mismo ocurre con el
indice de escolaridad, la cantidad de hombres de 14 y 24 años y la tasa de desempleo de 35 y 39 años. 
\\

Para las variables que refieren a cantidad de hombres o nivel de empelo de un tramo etario, hay que considerar que inicialmente se tiene información acerca de 
la tasa de desempleo en hombres de 14 a 24 años, sin embargo, la cantidad de hombres de 14 a 24 es años tiene una mayor fuerza explicativa.
\\

Con una interpertación objetiva, importa mas la cantidad de adolecentes y jovenes adultos, que la situación en el mercado laboral ya que la tasa de desempelo toma en cuenta la población 
economicamente activa (aquellas que con la edad suficiente buscan empelo), en cambio a edades mayores es crucial la situación labroral de este grupo de personas.
\\


Por último la estimación del regresor asociado a la \textit{Probabilidad de encarcelamiento} tiene una interpertación bastante fuerte ya que si comparamos dos estados con
...
sera sustancialmente menor ante una variación en esta medida.
\\

Algo importante a considerar es que para que se cumplieran los supuestos del modelo eliminamos tres observaciones, que en este caso refiere a que el modelo para tres estados de Estados
Unidos no esta siendo representado por lo que una posible solución para futuros trabajos sería aplicar \textit{Bootstrap residuals} en el modelo sin intervenir y aproximar la variabilidad de los estimadores.
