...
Continuando el ánalisis de los gráficos presentados anteriormente, se presenta en forma de tabla 
el resumen de las variables númericas. En el mismo se presenta el valor minimo y máximo de cada variable, medidas de tendencia central como lo es el primer y tercer cuartil, junto a la mediana. 
A su vez, para analizar la dispersión se incluye la media aritmetica y una medida de variabilidad de la misma, como es el Coeficiente de variación. 
...
Una vez presenatado el cuadro anterior, podemos agregar al ánalisis de la sección anterior y haciendo hincapie en que nuestras variables son estrcictamente positivas.
Utilizando el \textbf{CV} se puede apreciar que las variables de referida a población representa una gran variabilidad,algo similar ocurre con el \textit{Número de no caucásicos por cada 1000 habitantes}.
Esto podría determinar la forma en la que de incluye en el modelo, a continuación veremos la correlación parcial dos a dos de las posibles variables explicativas de nuestro modelo. 
...
Como una primera aproximación, se construye un modelo donde se incluyen todas las variables de nuestra tabla de datos, en concreto un modelo de regresión 
...
Recordando que el $R^{2}_{a}$ hace referencia a la variabilidad de que podemos explicar con nuestro modelo respecto para la tasa de criminalidad,
se puede considerar un valor \textit{aceptable}, mientras que a lo que refiere a la significación del modelo, considerando la prueba y el estadístico $F$ :
...
Siendo $SCE$ la suma de cuadrados explicados por la regresión y $RSE^{2}$ el cuadrado del error estandar de los Residuos, resultando para este caso particular $SCE = 5.525.982$ y $RSE^{2} = 43707,93$,
de esta manera podemos obtener el $F_{obs}$ que nos permite rechazar $H_{0}$ y así afirmar que el modelo es globalmente significativo.

Ahora continuación se pondra a prueba la siginificación de cada variable en forma marignal, los resultados se muestran el siguiente cuadro : 
...
lm(
  ...
  mutate(
    Variable = c("Intercepto",names(DatosTab)[-length(DatosTab)]),
    "Estimación" = estimate,
...
De esta manera se puede ver que no existe evidencia estadístca suficiente para rechazar que no existe una relación lineal de las variables incluidas para explicar la \textit{tasa de criminalidad} 
a exepción de la \textit{Cantidad de Hombres de 14-24 cada mil habitantes},\textit{la desigualdad en el ingreso} y la \textit{probabilidad de encarcelamiento}. 

En resumen, el modelo es globalmente significativo, pero en solo 3 de ellas podemos afirmar que sean significativas marginalmente en el modelo, es por esto que respetando el concepto de parsimonia 
y por una alta correlación entre variables, debemos de trabajar en la selección de las variables a utilizar.

\subsection{Aca iría la selección de modelos}
