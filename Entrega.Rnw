...
Voy a poner el gráfico de los valores atipicos
...
esos <- which(abs(si) > vc)
text(esos, si[esos]*0.95,esos)
@
...
En base a los test se puede afirmar que la distribución de los errores sigue una distribución Normal, a continuación haremos se hará el análisis de la homocedasticidad.

\subsection{Homosedasticidad}
