...
se considera al mismo como \textit{aceptable}. Por otro lado, haciendo referecia a la significación del modelo, se considera el siguiente test de hipótesis y el estadístico $F$ :
...
Consierando el problema previamente mencionado, se analizará la multicolinealidad (aproximada) de las variables independientes del modelo planteado. 
...
Como se menciono anteriormente, ante una posible relación lineal el determinante de la matriz $X^{T}X$ sería proximo a cero, siendo el determinante de la matriz inversa
...
Recordando el analisis de la sección anterior, el estado número \textbf{11}, \textbf{29} y \textbf{18} presentan problemas con la distancia de Cook, con este gráfico podemos afirmar que las observaciones \textbf{29} y 
\textbf{11} son influyentes y atipicas, mientras que para la número \textbf{18} podemos ver que es muy cercana a cero, la esperanza de los errores por lo que es solamente atípica.
...
En primer se hará un primer acercamiento con un gráfico de QQ-Plot, que muestra la similitud de los percentiles de la distribución normal téorica y la observada en nuestros errores, por último un gráfico de residudos contra los valores predichos para ver si existe algun patrón en su recorrido. 
...
la distribución se asemeja a una distribución normal, a exepción de la observación 17, 20 y 21, lo mismo se puede apreciar en gráfica que compara la distribución de los errores
...
Para poder afirmar que se cumple el supuesto de normalidad de los errores se relizan, tres pruebas:
...
Terminados ya todos los procesor referentes a la refinación del modelo completo original, se llega al denominado \textbf{Modelo Final}.El mismo incluye las siguientes variables:
\begin{itemize}
...
De esta manera, puede verse que ante una variación en un peso en el gasto de la policia la \textit{Tasa de Criminalidad} se esperaría que la tasa de criminalidad aumente en 11.50, dejando
...
Realizando una interpertación objetiva, en el modelo influye más la cantidad de adolecentes y jovenes adultos, que la situación del mercado laboral a la hora de estimar la tasa de delincuencia.
\\

Por último la estimación del regresor asociado a la \textit{Probabilidad de encarcelamiento} tiene una interpertación bastante fuerte, ya que si se comparan dos estados con
