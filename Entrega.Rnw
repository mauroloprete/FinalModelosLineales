...
En base a esto, podemos afirmar que nuestro modelo presenta problemas con la colinealidad y es por esto
que continuaremos con la selección a pasos por el método Stepwise, también que hay que recordar que en el caso de
seleccionar variables por la correlación dos a dos entre ellas y quedarse con aquellas que tienen una correlación mas alta
con $y$ es un grave error ya que nos estamos olvidando del efecto que puede tener con las demás variables. 

\subsection{Método Stepwise}

El método de Stepwise (basado en el \textit{F-Test}) comienza seleccionado aquella que tiene una mayor correrlación con la variable \textbf{y},
la segunda es aquel modelo que con la variable que se incluyó en el paso anterior maximiza el coeficiente de determinación siguiendo iterando hasta
que no se cumpla con el criterio de entrada. 
...
Tomando como regla empírica el valor de $\frac{4}{n-k-1} = 4/40$ puede verse que las observaciones \textbf{11}, \textbf{29} (de manera excesiva) y \textbf{17} sobrepasan la regla estipulada.
...
a exepción de la observación 40,11 y 6.
