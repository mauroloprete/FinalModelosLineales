...
\begin{titlepage}
  ...
  \vspace{3cm}
  \includegraphics[width=14cm]{Caratula.png}
  \vfill
  \vfill
  {\scshape\LARGE Modelos Líneales \par}
  {\scshape\Large Trabajo final \par}
  \vspace{0.5cm}
  {\scshape\Large Docentes : Laura Nalbarte y Fernando Massa \par}
  \vspace{3cm}
...
El objetivo de este informe es la construcción de un modelo de regresión lineal múltiple que explique la tasa de criminalidad en USA (número se ofensas reportadas a la policía por habitante).
\\
...
En el transcurso del texto se pondrán a prueba las distintas hipótesis centrales del modelo, tales como la normalidad de los errores,ausencia de multicolinealidad y la homocedasticidad.
\\
...
\textbf{Ed}     & Indice que refleja la escolaridad del estado                                                                                                          & Cuantitativa                                \\ \hline
...
Como se puede apreciar en los histogramas  presentados anteriormente y haciendo uso de la tabla (más precisamente del \textbf{CV}) es claro que las variables, de manera generalizada, presentan una variabilidad baja.
\\
...
DatosTab %>%
  ...
  kbl(
    booktabs = T, 
    caption = "Medidas descriptivas para variables numéricas"
  ) %>% 
  kable_styling(
...
Esto es en principio coherente, a pesar de ello y trás el análisis de distintos panoramas, el caso partícular de \textbf{Ineq} y \textbf{GDP} dejó en claro que esta técnica fue en un principio apresurada. Si bien las variables comparten gran nivel de información, trás la aplicación de las técnicas de \textbf{Forward,Backwards,Stepwise} ambas variables eran incluídas en el modelo en todo escenario.
\\
...
Como veremos en las siguientes secciones del documento, el caso de \textbf{Ineq} y \textbf{GDP} es partícular. Como se pensó en un principio, en el resto de las duplas muy probablemente sobreviva una sola de las variables (si no es que ambas son descartadas).
...
se considera al mismo como \textit{aceptable}. Por otro lado, haciendo referencia a la significación del modelo, se considera el siguiente test de hipótesis y el estadístico $F$ :
...
Siendo $SCE$ la suma de cuadrados explicados por la regresión y $RSE^{2}$ el cuadrado del error estándar de los residuos, resulta para este caso partícular $SCE = 5.525.982$ y $RSE^{2} = 43707,766$,
...
ModeloCompleto %>% 
  ...
  kbl(
    booktabs = T, 
    caption = "Estimación,error estándar y test individual del modelo completo ",
    escape = FALSE
  ) %>% 
  kable_styling(
    latex_options = c("striped", "hold_position"),
...
\newpage
A partir del cuadro presentado y conforme a los tests realizados, se ve claramente que son tan solo 4 son las variables que de manera independiente (\textbf{y muy importante, en presencia de todas las demás}) logran un aporte significativo al momento de explicar el comportamiento de la tasa de criminalidad.
\\

Ellas son \textbf{M},\textbf{PO1},\textbf{U2} y por último \textbf{Prob*100}.
\\

¿Significa esto que se debe descartar el resto de las variables y plantear un modelo caracterízado por tan solo las 4?, la respuesta es \textbf{no}. 
\\
...
Considerando el problema previamente mencionado, se analizará la multicolinealidad (aproximada) de las variables independientes del modelo planteado. 
...
haciendo que las estimaciones varíen ante pequeñas perturbaciones en las observaciones, logrando así que predicciones sean menos confiables. 
\\

Estamos frente a un problema de multicolinealidad aproximada cuando es posible afirmar que existe una relación lineal entre las variables explicativas. 
El término aproximado refiere al hecho de que en el caso de que se cumpla el fenómeno de forma exacta, la matriz no sería invertible y no existirían estimaciones únicas de los regresores
(Teorema de Gauss Markov). A su vez no se obtendrían estimadores eficientes (insesgados y de mínima varianza).
...
Como se menciono anteriormente, ante una posible relación lineal el determinante de la matriz $X^{T}X$ sería próximo a cero, siendo el determinante de la matriz inversa
demasiado grande. Es decir para un $\sigma^{2}$ fijo, la incertidumbre sería demasiado alta.
\\

Como se vió previamente, el modelo presentado hasta el momento es globalmente significativo, sin embargo tan solo 4 de las variables consideradas son significativas de manera independiente (presente todas las demás), es por esto que se cuantificará la intensidad de la multicolinealidad con el \textbf{Factor de inflación de varianza}.
...
Donde $R^{2}_{j}$ hace referencia al coeficiente de determinación de una regresión que intenta establecer una relación lineal de $X_{j}$ con las demás variables explicativas.
...
Con base en base a esto, podemos afirmar que este modelo presenta problemas de multicolinealidad y es por esto
...
ModeloRed %>%  
  ...
  kbl(
    booktabs = T, 
    caption = "Estimación,error estándar y test individual tras aplicar el método Stepwise ",
    escape = FALSE
  ) %>% 
  kable_styling(
    latex_options = c("striped", "hold_position"),
...
Una vez realizado dicho método\footnote{Se pueden encontrar las salidas en el anexo.} podemos encontrarnos con un modelo con 6 variables,
todas de ellas significativas y sin problemas de multicolinealidad, además de tener un $R^{2}_{a}$ de 
\Sexpr{ModeloRed %>% glance() %$% adj.r.squared %>% round(2)} mayor que el del modelo completo \Sexpr{ModeloCompleto %>% glance() %$% adj.r.squared %>% round(2)}.

Para continuar con el diagnostico del modelo, en la siguiente sección se hará un estudio de 
las observaciones atípicas y/o influyentes.

\newpage
\subsection{Búsqueda de observaciones influyentes}
...
Se analizaran aquellas observaciones que son atípicas, es decir aquellas que tienen una dispersión mayor a las demás, pero no 
hacen variar las estimaciones de los regresores.
\\

Para detectar observaciones atípicas, se construye un vector con los residuos studentizados del modelo y se compara con el umbral definido por
una distribución t y en el caso que la superen, se consideran atípicos.

Se definen los residudos studentizados como :

\begin{equation*}
  t_{i} = \frac{e}{\sqrt{\hat{\sigma^{2}_{-i}}(1-h_{ii})}}
\end{equation*}

Una vez calculado los residuos de esta manera, podemos definir a los outliers como aquellas observaciones que sobrepasen el umbral de 
una distribución $t^{df.res}_{1-0.01/2}$ (lineas continuas horizontales en el gráfico)
...
Recordando el análisis de la sección anterior, el estado número \textbf{11}, \textbf{29} y \textbf{18} presentan problemas con la distancia de Cook, con este gráfico podemos afirmar que las observaciones \textbf{29} y 
\textbf{11} son influyentes y atípicas, mientras que para la número \textbf{18} podemos ver que es muy cercana a cero, la esperanza de los errores por lo que es solamente atípica.
...
Uno de los supuestos fundamentales al momento de construír este modelo lineal, refiere a la distribución de los errores, en este caso se asume que siguen una distribución normal.
Esto permite realizar las pruebas anteriormente presentadas a lo largo de este trabajo, en el caso que este supuesto no se cumpla los estadísticos no reflejarán 
lo que se desea probar (su distribución no será la esperada), es por esto que es de crucial importancia chequear la distribución de los errores.
\\

En primer se hará un primer acercamiento con un gráfico de QQ-Plot, que muestra la similitud de los percentiles de la distribución normal teórica y la observada en nuestros errores, por último un gráfico de residudos contra los valores predichos para ver si existe algún patrón en su recorrido. 


<<out.width = "0.65\\textwidth",fig.align = "center",fig.cap = "Normalidad: Residuos vs modelo y Normal QQ-Plot">>=

# Gráfico de errores y qq plot
ModeloRedInter %>% 
  autoplot(
    which = c(1,2)
  ) + 
...
# Distribución teórica vs normal 
...
ModeloRedInter  %>% 
  ...
  labs(
    title = "Valores teóricos y errores studentizados " ,
    x="x",
    y = "Densidad"
  ) +
  theme(
...
También se debe mencionar que no es posible encontrar un patrón en la dispersión de los errores, en cambio en la gráfica QQ-plot muestra como los valores centrales de
la distribución se asemeja a una distribución normal, a excepción de la observación 17, 20 y 21, lo mismo se puede apreciar en gráfica que compara la distribución de los errores
(que fue construida con la función \textit{geom density} de ggplot que calcula y dibuja la estimación de la densidad Kernel con valores por defecto) con una distribución normal,
teniendo problemas en valores particulares y una gran concentración en valores poco antes de cero. 
\\
...
\begin{itemize}
  ...
  \item {\textbf{Shapiro-Wilks:} Que plantea un estadístico que es una función de las estadísticas de órden de la distribución de los errores y se compara con un valor de tabla}
  \item {\textbf{Jarque Bera:} Se basa en los coeficientes de simetría y kurtosis de la muestra, que para una normal son 0 y 3 respectivamente y con estadístico $\chi^{2}_{2}$}
\end{itemize}
...
\begin{equation*}
\begin{matrix}
H_{0})\sigma_{i}^{2}=\sigma^{2},\: \forall i=1,...,6\\ 
H_{1})\text{No}\,  H_{0}
\end{matrix}
\end{equation*}

Para ello se hará uso de los test de Breusch-Pagan, White, y Glejser.
\\

El test de \textbf{White} consiste en la siguiente serie de pasos:
\begin{itemize}
\item {Se estima el modelo y luego se genera el vector de errores estimados.}
\item{Se ajusta el modelo $e^{2}=X\alpha+Z\beta+W\delta+\mu$, con Z matriz de productos cruzados se las variables originales, W contiene las variables originales al cuadrado. }
\item{Se calcula $R^{2}$ del modelo.}
\item{Se calcula el estadístico $nR^{2}$ que se distribuye $\chi _{k}$ con k cantidad de variables del modelo.}
\item{Se realiza el test.}
\end{itemize}

El test de \textbf{Breusch-Pagan} empleado se basa en:
\begin{itemize}
\item{Se supone que la heterocedasticidad se debe a todas las variables.}
\item{Se estima el modelo $Y=X\beta+\varepsilon$, en conjunto con $e$ y $\hat{\sigma}$.}
\item{Se ajusta el modelo $\frac{e}{\hat{\sigma}^{2}}=X\alpha+\mu$.}
\item{Se define $BP=\frac{SCE}{2}$ estadístico con distribución $\chi _{k}$ bajo H0 cierta.}
\item{Se testea.}
\end{itemize}

Finalmente el test de \textbf{Glejser} consiste en:
\begin{itemize}
\item {Se estima el modelo y luego se genera el vector de errores estimados.}
\item{Se calcula $\left | e_{i} \right |$ para toda variable.}
\item{Se ajustan los siguientes modelos:
\begin{equation*}
\left | e \right |=\alpha+\beta X+\mu
\end{equation*}
\begin{equation*}
\left | e \right |=\alpha+\beta \sqrt{X}+\mu
\end{equation*}
\begin{equation*}
\left | e \right |=\alpha+\beta \frac{1}{X}+\mu
\end{equation*}}
\item{Se selecciona la regresión con el ajuste más alto}
\item{Se testea $H_{0})\beta=0$, si se rechaza la hipótesis nula, se rechaza la homocedasticidad}
\end{itemize}
<<>>=
...
Trás la realización de los 3 tests, se confirma con gran nivel de seguridad que el modelo actual no presenta problemas con respecto al supuesto de la homocedasticidad.
\newpage
\section{Modelo Final}
\subsection{Expresión}

Terminados ya todos los procesos referentes a la refinación del modelo completo original, se llega al denominado \textbf{Modelo Final}.El mismo incluye las siguientes variables:
\begin{itemize}
\item{\textit{Gasto per cápita en policía en 1960} (\textbf{Po1}).} 
\item{\textit{Desigualdad en el ingreso} (\textbf{Ineq})}  
\item{\textit{Índice de la escolaridad del estado} (\textbf{Ed}).}
\item{\textit{cantidad de hombres entre 14 y 20 años de edad cada 1.000 habitantes.} 
(\textbf{M})}
\item{\textit{Tasa de desempleo urbana de hombres de 35 a 39 años por 1000 habitantes.} (\textbf{U2})}
\item{\textit{Probabilidad de encarcelamiento*100} (\textbf{Prob}).}
\end{itemize}
...
ModeloRedInter %>%  
  ...
  kbl(
    booktabs = T, 
    caption = "Estimación,error estándar y test individual tras aplicar el método Stepwise intervenido por observaciones influyentes",
    escape = FALSE
  ) %>% 
  kable_styling(
    latex_options = c("striped", "hold_position"),
...
relocate(
  ...
  kbl(
    booktabs = T,
    caption = "Comparación modelo completo vs reducido e intervenido",
    escape = FALSE
  ) %>% 
    kable_styling(
    latex_options = c("striped", "hold_position"),
...
De esta manera, puede verse que ante una variación en un peso en el gasto de la policía la \textit{Tasa de Criminalidad} se esperaría que la tasa de criminalidad aumente en 11.50, dejando
las demás variables constantes. También es preciso afirmar que en estados donde existe una mayor desigualdad en el ingreso, se espera que la tasa de criminalidad sea mayor, lo mismo ocurre con el
indice de escolaridad, la cantidad de hombres de 14 y 24 años y la tasa de desempleo de 35 y 39 años (cosa que se podía afirmar desde el análisis descriptivo se encuentra ahora cuantificada de manera más precisa). 
\\

Las variables relacionadas con la cantidad de hombres o nivel de empleo en un tramo etario dado, se debe considerar si bien se tiene información acerca de 
la tasa de desempleo en hombres de 14 a 24 años, la cantidad de hombres de 14 a 24 años por cada 1000 habitantes (independientemente de su condición laboral) tiene una mayor fuerza explicativa.
\\

Realizando una interpretación objetiva, en el modelo influye más la cantidad de adolescentes y jóvenes adultos, que la situación del mercado laboral a la hora de estimar la tasa de delincuencia.
\\

Por último la estimación del regresor asociado a la \textit{Probabilidad de encarcelamiento} tiene una interpretación bastante fuerte, ya que si se comparan dos estados con
la misma situación de Gasto,Desigualdad en el ingreso, Desempleo en Hombres de 35 a 39 años, cantidad de hombres de 14 y 24 años y con un mismo nivel educativo, la tasa de criminalidad
sera sustancialmente menor ante una variación positiva de esta variable.
\\

\subsection{Conclusiones}

De manera resumida y quizás en cierta parte reiterativa, el proceso de construcción del modelo se basó en 3 pilares fundamentales:

\begin{itemize}
\item{\textbf{Fidelidad en la representación:} se buscó construír un modelo que efectivamente logre explicar el comportamiento de la tasa de la manera más certera posible. Como se vé en el cuadro anterior, se llegó a una expresión cuyo ajuste es del 80,56\% apróximadamente.}
\item{\textbf{Balance:} se buscó una cantidad de variables adecuadas, la eliminación de parte de ellas permitió obtener un modelo para nada sobrecargado y sobre todo eficiente. A su vez, la eliminación de variables no fue desmedida, en todo momento una justificación teórica la respalda.}
\item{\textbf{Control:} si bien la eliminación de observaciones que perturban los supuestos clásicos del modelo puede ser absorbente, en un modelo construído a partir de una cantidad medianamente acotada de observaciones esto lleva sin ninguna duda a la pérdida de fidelidad representativa. Nuevamente, es claro que en el proceso estas decisiones tomaron tiempo de pensamiento y se basaron en los conceptos estudiados en clase. }
\end{itemize}

Un detalle final a tener nuevamente en consideración es la eliminación de las 3 observaciones que ponen en jaque a los supuestos planteados. Este modelo no se va adecuar "fielmente" a estos 3 estados, una posible solución para futuros trabajos sería aplicar \textit{Bootstrap residuals} en el modelo sin intervenir y aproximar la variabilidad de los estimadores. Esto es sin duda una herramienta que nos propusimos a explorar en el futuro.
...
\subsection{Gráfico descriptivo}
En esta gráfica se puede ver la correlación entre las variables presentadas ,un gráfico de dispersión y la distribución de la variable.
...
En este apartado se muestra la salida de la selección a pasos de Stepwise, se puede ver que el $\alpha$ de entrada y salida es igual a $0.15$, comienza
solo incluyendo la constante en el modelo, luego agrega \textbf{Po1} hasta llegar al séptimo paso y terminar el algorítmo.
...
Aquí se mostrará el código R utilizado para el trabajo, para reproducirlo solo hace falta incluir en el mismo directorio de trabajo
el archivo \textbf{USCrime.txt}. En el proceso se desplegarán las diferentes gráficas y tablas, así como también el modelo completo \textit{ModeloCompleto},
el modelo reducido \textit{ModeloRed} y el modelo reducido intervenido como \textit{ModeloRedInter} en el ambiente de trabajo.

El mismo puede encontrarse en \url{https://gist.github.com/mauroloprete/dcb1c0bba5215baa24311be4e77be8cb}.
