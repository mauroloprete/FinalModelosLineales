...
Trás esto, se decide que \textbf{Ineq} y \textbf{P02} no sean incluídos en el modelo inicial ya que se entiende que las mismas no tendrán un aporte significativo en presencia de sus pares.
\\
...
Otro aspecto a considerar es , ¿qué tan correlacionadas se encuentran las mismas variables con y?.

Un caso partícular es el de las variables \textbf{S01} y \textbf{NW} que claramente se encuentran relacionadas (con un coeficiente de correlación de 0,767). A su vez resalta como ambas variables tienen una correlación con \textbf{y} prácticamente despreciable
...
En base a lo último es claro que estas 2 variables no serán significativas para explicar a \textbf{y} y que en cualquier proceso de selección computarizado serán descartadas.
\\
...
Ellas son \textit{U1,Ineq y Prob}.
\\
