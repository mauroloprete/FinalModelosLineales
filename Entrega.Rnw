...
\newpage
A partir del cuadro presentado y conforme a los tests realizados, se ve claramente que son tan solo 3 son las variables que de manera independiente (\textbf{y muy importantemente, en presencia de todas las demás}) logran un aporte significativo al momento de explicar el comportamiento de la tasa de criminalidad.
\\

Ellas son \textbf{PO1}: Gasto pér capita en policía en 1960, \textbf{U2}: Tasa de desempleo Urbana de hombres
entre 35 y 39 años por 1000 habitantes y por último \textbf{Prob*1000} Probabilidad de encarcelamiento cada 1000 habitantes.
\\
...
esto es relevante ya que si existe una relación lineal en la matriz de diseño, estoimpactaría directamente en la varianza de los regresores $\beta_{k} = \left(X^{T}X\right)^{-1}X^{T}$, 
...
Tomando como regla empírica el valor de $\frac{4}{n-k-1}$ puede verse que las observaciones \textbf{19}, \textbf{29} (de manera excesiva) y \textbf{31} sobrepasan la regla estipulada.
...
Considerando los gráficos anteriormente mencionados podemos ver que la esperanza de los errores se mantiene
cercana a cero, a diferencia de algunas observaciones y encontramos un patrón en la dispersión de los errores,
en cambio en la gráfica QQ-plot podemos ver que en valores centrales de la distribución se asemeja a una distribución normal
a exepción de la observación 46 y 11.
